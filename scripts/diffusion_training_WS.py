...
#     resume = False
    ...
    if args["channels"] != "":
        in_channels = args["channels"]
    model = UViT(img_size = args['img_size'][0], patch_size=16, in_chans=args["channels"], embed_dim = args['base_channels'],
...
#             )
    ...
    if resume:
        if "UViT" in resume:
            model.load_state_dict(resume["UViT"])
        else:
            model.load_state_dict(resume["ema"])

        ema = UViT(img_size = args['img_size'][0], patch_size=16, in_chans=args["channels"], embed_dim = args['base_channels'],
                   depth=12, num_heads=args["num_heads"], mlp_ratio=4., qkv_bias=False, qk_scale=None, norm_layer=nn.LayerNorm, 
                   mlp_time_embed=False, num_classes=-1,
                   use_checkpoint=False, conv=True, skip=True)
