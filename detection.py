...
# from src.models.UModels.UNet import UNetModel
import cv2
import numpy as np
...
#                 )
        ...
        ano_mask = torch.tensor(cv2.threshold(ano_mask.cpu().numpy(), 0, 1, cv2.THRESH_BINARY)[1]).reshape(*mse.shape)
#         fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(8, 12))
        output1 = torch.cat((img, output.to(device)))
#         output1 = output1.reshape(8, 1, real.shape[-2], real.shape[-1])
#         print("output1", output1.shape)
        output2 = torch.cat((mse, ano_mask))
