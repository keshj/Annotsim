...
def init_datasets(ROOT_DIR, args):
    if args["dataset"] == 'mri':
    ...
    elif args["dataset"] == 'brats21mri':
        training_dataset = MRIDataset(
            ROOT_DIR=f'{ROOT_DIR}DATASETS/ANNO2021B/', img_size=args['img_size'], transform = True, 
        ...
        training_dataset = AnnoDataset(args['dir'], input_size = args['img_size'], channels = args['channels'])
        testing_dataset = AnnoDataset(args['dir'], set = "test", input_size = args['img_size'], channels = args['channels'])
        ...
        training_dataset = HnABRATS(ROOT_DIR, is_train=True, num_mod = args["channels"])
        testing_dataset = HnABRATS(ROOT_DIR, is_train=False, num_mod = args["channels"])
...
class HnABRATS(torch.utils.data.Dataset):
    def __init__(self, ROOT_DIR, img_size = (256, 256), is_train = True, transform = None, num_mod = 4):     
        self.D = num_mod
        self.transform = transforms.Compose(
            [transforms.ToPILImage(),        
             transforms.Resize(img_size, transforms.InterpolationMode.BILINEAR),
